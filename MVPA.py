...
class MVPA(object):
    def __init__(self,baseDir,ID=None,event_ids=None,baseline=None,balance=True,selection='ALL',condName='', dimension='timetime'):
        ...
        if ID!=None:
            ...
            if self.dimension == 'timefrequency':
                self.paramsFile = glob.glob(os.path.join(self.subDir, self.subject + '*' + str(self.index) + '*_epo-tfr.csv'))[-1]
                ...
                self.epochFilename = glob.glob(os.path.join(self.subDir, self.subject + '*' + str(self.index) + '*_epo-tfr.h5'))[-1]
    ...
    def prepDecoding(self, method, crossclass=False, times=[-0.2, 1.0], decim=8, supra=False):
        ...
        if self.dimension == 'timetime':
            ...
            freqs = list(range(self.epochsClass1.freqs.shape[0]))
        ...
        for f in range(len(freqs)):
            print('Decoding frequency {} out of {}'.format(str(f+1),str(len(freqs))))
        ...
        if self.dimension == 'timefrequency':
            df.columns = self.epochsClass1.freqs

        df.insert(0,'times',self.epochsClass1.times) 
        df.to_csv(os.path.join(self.decDir, '_'.join((self.subject, str(self.index),method,self.condName  , self.dimension, self.selection + ('_supra' * supra) +'.csv'))),index=False)
    ...
    def crossDec(self, method, y_train, X_train, y_test, X_test, crossclass=False):
        ...
        dp = { 'method': method, 'dec': time_decod,
                  'cond': self.condName, 'supra': False, 
                  'times': self.epochsClass1.times, 'chance': 0.5,'trials':X_train.shape[0],'freqs':0}
 
        return scores, dp 

    def plotDec(self,scores,dp,name=''):
        ...
        if len(scores.shape)>1:#[0] == scores.shape[1]:
            ...
            plt.savefig(fname = os.path.join(plotDir,'_'.join((self.subject,str(self.index),dp['method'],self.condName))+(dp['supra']*'_supra' + '_' + nametag +'.pdf')),format= 'pdf')
            ...
            plt.savefig(fname = os.path.join(plotDir,'_'.join((self.subject,str(self.index),dp['method'],self.condName))+(dp['supra']*'_supra_' + name + '_diagonal.pdf')),format= 'pdf')
    ...
    def groupLevel(self, subs, idx, task, method,dimension='timetime', cond ='*', supra = False, selection = 'ALL',**kwargs):   
        ...
        if not os.path.isdir(plotDir):
            os.makedirs(plotDir)
        # Load in data
        print('Now analyzing group effects for {} session {}'.format(task,str(idx)))
        Mat = np.array([])
        # shell()
        for s in subs:
            classDir = self.baseDir + 'Proc/' + task + '/' + s + '/decoding/'
            try:
                filename = glob.glob(os.path.join(classDir, ('_').join((s, str(idx), method, cond , dimension, selection + (' supra' * supra) + '.csv'))))[-1]
            except:
                continue
            ...
            if dimension == 'timetime':
                dat = dat.values[:,1]
                Mat = np.append(Mat,dat)    
            elif dimension == 'tempGen' or dimension == 'timefrequency':
                freqs = np.linspace(2,30,15)#np.array(dat.columns)
                dat = dat.values[:,1:]
                Mat = np.append(Mat,dat)    

        # reshape to get subject x data (time/timextime) array
        if dimension == 'timetime':
            allDat = Mat.reshape(int(Mat.shape[0]/dat.shape[0]),dat.shape[0])
        elif dimension == 'tempGen' or dimension == 'timefrequency':
            allDat = Mat.reshape(int(Mat.shape[0]/dat.shape[0]/dat.shape[1]),dat.shape[0],dat.shape[1])
        ...
        freqlabels = list(linspace(0,len(freqs)-1,4,dtype=int)) if dimension == 'timefrequency' else 0
        ...
        if dimension == 'timetime':
            ...
            extent = [0,dat.shape[0],0,dat.shape[1] ]#times[0],times[-1],2,40]
            ...
            im = ax.imshow(allDatMean,cmap = 'RdBu_r', interpolation = 'lanczos',origin = 'lower',vmin=0.4, vmax = 0.6, aspect='auto',extent=extent)
            ...
            for l in cbar.ax.yaxis.get_ticklabels():
                ...
                l.set_size(12)
            # shell()
            plt.contour(t_thresh,[0.5],colors='black',linestyles='solid',linewidths=[2],levels=0,origin='lower',
                extent=[0, t_thresh.shape[0],0, t_thresh.shape[1]])
            xticks(timelabels, np.around(times[timelabels],decimals=2),fontsize=12,fontweight='bold')
            
            if dimension=='tempGen':
                yticks(timelabels, np.around(times[timelabels],decimals=2),fontsize=12,fontweight='bold')
            else:
                yticks(freqlabels, np.around(freqs[freqlabels],decimals=1),fontsize=12,fontweight='bold')

            xlabel('Testing time (s)',fontsize=14,fontweight='bold')
            yLab = 'Training time (s)' if dimension=='tempGen' else 'Frequency (Hz)'
            ylabel(yLab,fontsize=14,fontweight='bold')

        plt.title('{}'.format(cond),fontsize=14,fontweight='bold')
        # plt.tight_layout()

        plt.savefig(fname = plotDir +'/'+ '_'.join((method, str(idx), cond, dimension,  (' supra' * supra) + '_' + selection + '.pdf')),format= 'pdf')
        plt.close()
